...
public class ThreadedUDP : MonoBehaviour {
    ...
    float syncFlashTime = 133280; // SyncFlash time in cycles for a 16mhz clock
    ...
    // Is this returning in degrees or radians? Probably radians... Is that important?
    ...
    public void NewtonsNewMethod(){
        ...
        // Calculating results of initial point substituted into original system of non-linear equations
        float gRA, gRB, gRC;
        for (int i = 0; i < 50; i++) {
            // Move all data declorations out of for loop
            ...
            sm2_RB = p2_RB;
            sm2_RC = p2_RC;
            sm2_const = (-tempRA * p2_RA) + (-tempRB * p2_RB) + (-tempRC * p2_RC);
            ...
            //The matrix to solve will have the following form...Do a jacobian solve on it...
            //sm1_RA + sm1_RB + sm1_RC + sm1_const = gRA
            //sm2_RA + sm2_RB + sm2_RC + sm2_const = gRB
            //sm3_RA + sm3_RB + sm3_RC + sm3_const = gRC
        ...
        if (firstPosition == false){
            firstPosition = true;
            //A_offsetX = m_theta1 * Mathf.Rad2Deg;
    ...
    //newPos = new Vector3((theta1Deg - A_offsetX), (b1Deg - A_offsetY), (m_RA - A_offsetZ));
